      ...
      <SidebarHeader className="border-b p-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 relative flex-shrink-0">
            <Image src={logo} alt="Townspark" fill className="object-contain" />
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-lg truncate">Townspark</h2>
      ...
      <SidebarFooter className="border-t p-3">
        ...
        <div className="flex items-center gap-3 mb-3 p-2 rounded-2xl bg-muted/30 border border-border/50">
          <Avatar className="w-10 h-10 border-2 border-background shadow-sm">
            <AvatarImage
              src={
                getUserProfilePic()?.startsWith("http")
                  ? getUserProfilePic()
                  : getUserProfilePic()
                    ? `${process.env.NEXT_PUBLIC_API_URL}${getUserProfilePic()}`
                    : undefined
              }
              alt={getUserFullName()}
            />
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground truncate font-bold opacity-60">
              {getUserEmail() ||
                (getUserId() ? `ID: ${getUserId()}` : "Guest Account")}
            </p>
          </div>
        </div>
        <Button
